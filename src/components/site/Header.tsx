...
export function Header() {
  ...
  const wa = whatsappLink(settings?.whatsapp_number, "Hello, I would like to know more.");
      ...
      <div className="section-shell flex h-16 items-center justify-between gap-4 md:h-20">
        <Link to="/" className="flex min-w-0 items-center gap-3" onClick={() => setOpen(false)}>
          {logo ? (
            ...
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary font-display text-sm font-semibold text-primary-foreground">
              {(settings?.business_name ?? "F").slice(0, 2).toUpperCase()}
            </span>
          )}
          <span className="truncate font-display text-lg font-semibold tracking-tight md:text-xl">
            {settings?.business_name ?? "fts88994"}
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          {wa ? (
            <a href={wa} target="_blank" rel="noopener noreferrer" className="hidden sm:inline-flex">
      ...
      {open ? (
        <div className="border-t border-border bg-background lg:hidden">
          <nav className="section-shell flex flex-col py-3">
            ...
            {wa ? (
              <a href={wa} target="_blank" rel="noopener noreferrer" className="mt-2">
