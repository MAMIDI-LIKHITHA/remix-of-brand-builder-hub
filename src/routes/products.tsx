...
export const Route = createFileRoute("/products")({
  head: () => ({
    meta: [
      { title: "Products — fts88994" },
      {
        name: "description",
        content:
          "Browse the full fts88994 product catalogue, filter by category and enquire on WhatsApp.",
      },
      { property: "og:title", content: "Products — fts88994" },
      {
        property: "og:description",
        content: "Browse the full product catalogue and enquire on WhatsApp.",
      },
    ],
  }),
...
function ProductsPage() {
  ...
  const visible = useMemo(() => {
    ...
    let list = products.filter((p) => p.is_published);
    if (q) {
      list = list.filter((p) =>
        [p.name, p.short_description, p.full_description]
          .filter(Boolean)
  ...
  const showTools = published.length > 3;
  ...
  return (
    <SiteLayout>
      <PageHeader
        eyebrow="Products"
        title="Our products"
        description="Everything currently available. Tap any item for full details or enquire straight away on WhatsApp."
      />
      ...
      <section className="section-shell py-12 md:py-16">
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            body="Products added from the admin panel will appear here automatically."
          />
        )}
      </section>
    </SiteLayout>
