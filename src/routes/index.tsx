...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "fts88994 — Products & Services in Qatar and Dubai" },
      {
        name: "description",
        content:
          "fts88994 — browse our products and services and message us directly on WhatsApp. Serving Qatar and Dubai.",
      },
      { property: "og:title", content: "fts88994 — Products & Services in Qatar and Dubai" },
      {
        property: "og:description",
        content: "Browse our products and services and message us directly on WhatsApp.",
      },
    ],
  }),
  component: HomePage,
...
function HomePage() {
  ...
  const featuredServices = (services.filter((x) => x.is_featured).length
    ? services.filter((x) => x.is_featured)
    : services
  ).slice(0, 3);
  ...
  return (
    <SiteLayout>
      ...
      <section className="relative overflow-hidden bg-ink text-ink-foreground">
        ...
        <div className="section-shell relative py-24 md:py-36">
          <div className="max-w-2xl">
            {s?.tagline ? (
              <p className="font-arabic mb-5 text-base text-ink-foreground/85">{s.tagline}</p>
            ) : null}
            <h1 className="text-4xl leading-[1.05] sm:text-5xl md:text-6xl">
              {s?.hero_heading ?? s?.business_name ?? "fts88994"}
            </h1>
            {s?.hero_subheading ? (
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              <Link to="/products">
                <Button size="xl" variant="hero">
                  {s?.hero_primary_cta ?? "View Products"} <ArrowRight className="size-4" />
                </Button>
              </Link>
              <Link to="/services">
                <Button size="xl" variant="onInk">
                  {s?.hero_secondary_cta ?? "Our Services"}
                </Button>
              </Link>
              {wa ? (
      ...
      <section className="section-shell py-20 md:py-28">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl">{s?.about_heading ?? "About us"}</h2>
            {s?.about_body ? (
        ...
        <section className="section-shell py-20 md:py-28">
          <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl">How we help</h2>
            </div>
            <Link to="/services">
              <Button variant="outline">
        ...
        <section className="section-shell pb-20 md:pb-28">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl">A closer look</h2>
            </div>
            <Link to="/gallery">
              <Button variant="outline">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {gallery.slice(0, 8).map((g) => (
              <Link key={g.id} to="/gallery" className="surface-card group overflow-hidden">
                <ImageFrame src={g.image_url} alt={g.title ?? ""} className="aspect-square" />
              </Link>
            ))}
          </div>
        </section>
      ...
      <section className="section-shell pb-4">
        <div className="grain-hero surface-card flex flex-col items-start gap-6 overflow-hidden bg-ink px-8 py-14 text-ink-foreground md:px-14">
          <div>
            <h2 className="text-3xl md:text-4xl">{s?.cta_heading ?? "Get in touch"}</h2>
            {s?.cta_body ? (
              <p className="mt-4 max-w-xl text-base text-ink-foreground/80">{s.cta_body}</p>
            ) : null}
          </div>
